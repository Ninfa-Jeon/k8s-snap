...
var chartStmts = map[string]int{
	...
	"exists":       MustPrepareStatement("helm-charts", "exists.sql"),
}

// HelmChartExists checks if a helm chart exists in the database by name and version.
func HelmChartExists(ctx context.Context, tx *sql.Tx, name string, version string) (bool, error) {
	selectTxStmt, err := cluster.Stmt(tx, chartStmts["exists"])
	if err != nil {
	...
	err = selectTxStmt.QueryRowContext(ctx, name, version).Scan(&exists)
	if err != nil {
		return false, fmt.Errorf("failed to query helm chart: %w", err)
	}
